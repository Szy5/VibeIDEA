...
def send_email(sender:str, receiver:str, password:str,smtp_server:str,smtp_port:int, html:str,):
    ...
    # 465 端口必须先 SSL；587 端口用 STARTTLS。失败时按顺序回退尝试
    server = None
    if smtp_port == 465:
        try:
            server = smtplib.SMTP_SSL(smtp_server, smtp_port)
        except Exception as e:
            logger.debug(f"SMTP_SSL(465) failed: {e}, try STARTTLS.")
            try:
                server = smtplib.SMTP(smtp_server, smtp_port)
                server.starttls()
            except Exception as e2:
                logger.warning(f"STARTTLS on 465 failed: {e2}")
                raise
    else:
        try:
            server = smtplib.SMTP(smtp_server, smtp_port)
            ...
            logger.debug(f"STARTTLS failed: {e}, try SSL.")
            try:
                server = smtplib.SMTP_SSL(smtp_server, smtp_port)
            except Exception as e2:
                logger.warning(f"SMTP_SSL fallback failed: {e2}")
                raise
